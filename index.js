...
bot.on('/welcomepromise', (msg) => {
    console.log(`/welcomepromise triggered`)
    let messages = [
        `Welcome Peter Fry `,
        `Welcome Stu `,
        `Welcome Will `,
    ...
    messages.unshift(`Welcome ${ msg.from.first_name } `);
    let messagesToSend = []

    Promise.each(messages, welcome => {
        const ranNum = Math.floor(Math.random() * 20) + 1
        let emojiBoi = ((ranNum < 6) ? "" : "\u{1F602}")
        emojiBoi += (ranNum < 12) ? '' : "\u{1F602}"
        emojiBoi += (ranNum < 18) ? '' : "\u{1F602}"
        return bot.sendMessage(msg.chat.id, welcome + emojiBoi);
    })
...
bot.on('newChatMembers', (msg) => {
    ...
    try {
        for (newMember in msg.new_chat_members) {
            ...
            messages.unshift(`Welcome ${ msg.new_chat_members[newMember].first_name }`)
